...
fs.readdir(`${__dirname}/tests`,(err, testNames)=>{
    ...
    testNames.forEach(t=>{
        ...
        var test ={
           code,
           IO:require(`${__dirname}/tests/${t}/IO.json`)
        };
        /*PERF*/ _P.setLoad.stop.push(Date.now());
        console.log(`\t Loaded ${t} with ${test.code.length} lines & ${test.IO.out.length} test cases, running...`);
        ...
        for(var testCase=0; testCase<test.IO.in.length; testCase++){
            ...
            if(chip.localEnv.global[':done'].value!=0){ //Chip finished this test case
                ...
                throw `Reached Tick limit on ${t} case ${testCase}`;
            }
