...
export function lex(line,linenumber=0){   //TODO add more info for exceptions (eg. line number)
    ...
    proc:for(var i=0; i<chars.length; i++){
        ...
        if(c==' '||c=="\n"||c=="\t"){   //this should only be one line, so ignore extra tabs & newlines
            ...
            if(c=="="){ //subtype 1 | 2
                if(prev.type==0){
                    if(prev.subtype==0){
                        ...
                        lexed.push({type:0, subtype:-1, value:"=", pos:{l:linenumber,c:i-1}});
                    }
                }else if(prev.value=="!"){
            ...
            if(chars[i]!='"') {
                //throw "No end of string!" //TODO better exceptions with lines etc
            ...
            let value = c;
            let pos = i;
            while(++i<chars.length && chars[i].match(voodoo)) value+=chars[i];
            i--
            value=value.toLowerCase();
            lexed.push({type:6, subtype:0, value, pos: {l:linenumber, c:pos}});
        } else {
            lexed.push({type:7, subtype:0, value:c, pos: {l:linenumber, c:pos}});
